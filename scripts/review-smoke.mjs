...
try {
  ...
  expectStatus(await request(`/api/users/${insideUser.data.id}/department`, { token:departmentAdminToken, method:'PATCH', body:{ departmentId:childDepartmentOne.data.id } }), 200, '部门管理员调换本部门普通用户')
  expectStatus(await request(`/api/users/${insideUser.data.id}/department`, { token:departmentAdminToken, method:'PATCH', body:{ departmentId:2 } }), 403, '部门管理员将用户调出管理范围')
  expectStatus(await request(`/api/users/${createAdmin.data.id}/department`, { token:departmentAdminToken, method:'PATCH', body:{ departmentId:childDepartmentOne.data.id } }), 403, '部门管理员调整管理员部门')
  ...
  expectStatus(await request(`/api/users/${promotedUser.data.id}/department`, { token:adminToken, method:'PATCH', body:{ departmentId:4 } }), 200, '系统管理员调整部门管理员所属部门')
