...
function DepartmentsView({ data, reload, notify }) {
  ...
  return <section className="page enter">
    ...
    {deleteTarget && <div className="modal-wrap"><div className="scrim" onClick={() => setDeleteTarget(null)}/><div className="modal confirm-modal danger-confirm"><div className="confirm-icon"><Trash2 size={23}/></div><div className="modal-head"><div><span className="eyebrow">删除部门</span><h2>确定删除“{deleteTarget.name}”？</h2></div><button className="icon-button" onClick={() => setDeleteTarget(null)}><X size={20}/></button></div><p className="modal-intro">只有没有下级部门、且没有所属用户的部门才能删除。删除后无法恢复。</p><div className="modal-actions"><button className="ghost" onClick={() => setDeleteTarget(null)}>取消</button><button className="danger-button" onClick={remove}>确认删除部门</button></div></div></div>}
  </section>
}
