...
function UsersView({ data, reload, notify }) {
  ...
  const copyInviteLink = async (link = createdLink) => {
    try {
      if (navigator.clipboard?.writeText) await navigator.clipboard.writeText(link)
      else {
        const input = document.createElement('textarea'); input.value = link; input.style.position = 'fixed'; input.style.opacity = '0'; document.body.appendChild(input); input.select(); document.execCommand('copy'); input.remove()
      }
      notify('邀请链接已复制')
  ...
  const openPasswordReset = () => { setResetTarget(manageTarget); setResetResult(null); setManageTarget(null) }
  ...
  return <section className="page enter">
    ...
    <div className="user-table">
      ...
      {visibleUsers.map(u => {
        ...
        const canOpenSettings = !targetIsSystemAdmin && (isSystemAdmin || u.role === 'user')
...
function DepartmentsView({ data, reload, notify }) {
  ...
  const remove = async () => { try { await api(`/api/departments/${deleteTarget.id}`, { method:'DELETE' }); setDeleteTarget(null); await reload(); notify('部门已删除') } catch(error) { notify(error.message) } }
  ...
  return <section className="page enter">
    ...
    {!!tree.length && <div className="department-panel"><div className="department-head"><span>部门层级</span><span>{scopeDepartments.length} 个部门</span></div>{tree.map(node => <DepartmentRow key={node.id} node={node} depth={0} rootId={isSystemAdmin ? null : data.currentUser.department_id} add={setAddingTo} edit={edit} remove={setDeleteTarget} users={data.users}/>)}</div>}
    ...
    {deleteTarget && <div className="modal-wrap"><div className="scrim" onClick={() => setDeleteTarget(null)}/><div className="modal confirm-modal danger-confirm"><div className="confirm-icon"><Trash2 size={23}/></div><div className="modal-head"><div><span className="eyebrow">删除部门</span><h2>确定删除“{deleteTarget.name}”？</h2></div><button className="icon-button" onClick={() => setDeleteTarget(null)}><X size={20}/></button></div><p className="modal-intro">需先处理下级部门和所属用户。流程模板中的该部门会改为“未指定部门”，已创建待办仍保留原部门名称。</p><div className="modal-actions"><button className="ghost" onClick={() => setDeleteTarget(null)}>取消</button><button className="danger-button" onClick={remove}>确认删除部门</button></div></div></div>}
  </section>
}
