...
function UsersView({ data, reload, notify }) {
  ...
  const changeDepartment = async e => { e.preventDefault(); try { await api(`/api/users/${departmentTarget.id}/department`, { method:'PATCH', body:JSON.stringify({ departmentId:departmentTarget.nextDepartmentId }) }); setDepartmentTarget(null); await reload(); notify('用户所属部门已更新') } catch(error) { notify(error.message) } }
  ...
  return <section className="page enter">
    ...
    <div className="user-table">
      ...
      {visibleUsers.map(u => {
        ...
        const canChangeDepartment = !targetIsSystemAdmin && (isSystemAdmin || u.role === 'user')
        return <div className="table-row" key={u.id}><span className="user-cell"><i>{u.name.slice(-1)}</i><b>{u.name}</b></span><span>{u.username}</span><span>{u.department_name || '—'}</span><span>{targetIsSystemAdmin ? '系统管理员' : u.role === 'department_admin' ? '部门管理员' : '普通用户'}</span><span className="user-actions">{isCurrent && <small className="current-user">当前登录</small>}{!isCurrent && targetIsSystemAdmin && <small className="protected-user">不可修改</small>}{canChangeDepartment && <button className="edit-button" title="调换部门" aria-label={`调整${u.name}的所属部门`} onClick={() => setDepartmentTarget({ ...u, nextDepartmentId:String(u.department_id || '') })}><Building2 size={15}/></button>}{canChangeRole && <button className="edit-button" title="修改角色" aria-label={`修改${u.name}的角色`} onClick={() => setRoleTarget({ ...u, nextRole:u.role === 'department_admin' ? 'user' : 'department_admin' })}><Settings size={15}/></button>}{canReset && <button className="reset-button" title="重置密码" aria-label={`重置${u.name}的密码`} onClick={() => { setResetTarget(u); setResetResult(null) }}><KeyRound size={15}/></button>}{canDelete && <button className="delete-button" title="删除用户" aria-label={`删除${u.name}`} onClick={() => setDeleteTarget(u)}><Trash2 size={16}/></button>}</span></div>
      })}
    </div>
    ...
    {departmentTarget && <div className="modal-wrap"><div className="scrim" onClick={() => setDepartmentTarget(null)}/><form className="modal" onSubmit={changeDepartment}><div className="modal-head"><div><span className="eyebrow">部门归属</span><h2>调换“{departmentTarget.name}”的部门</h2></div><button type="button" className="icon-button" onClick={() => setDepartmentTarget(null)}><X size={20}/></button></div><Field label="目标部门"><select required value={departmentTarget.nextDepartmentId} onChange={e => setDepartmentTarget({...departmentTarget, nextDepartmentId:e.target.value})}><option value="">请选择</option>{options.map(d => <option value={d.id} key={d.id}>{d.label}</option>)}</select></Field><p className="modal-intro">当前部门：{departmentTarget.department_name || '未分配'}。调整后，该用户会按照新部门参与流程分享范围。</p><button className="primary full" disabled={String(departmentTarget.department_id || '') === String(departmentTarget.nextDepartmentId)}>确认调换部门</button></form></div>}
